	...
	public function clone():DOMSymbolInstance
	{
		...
		if (color != null)
			duplicate.color = new Color(color.redMultiplier, color.greenMultiplier, color.blueMultiplier, color.alphaMultiplier, color.redOffset,
				color.greenOffset, color.blueOffset, color.alphaOffset);
		duplicate.libraryItemName = libraryItemName;
		duplicate.loop = loop;
		if (matrix != null)
			duplicate.matrix = new Matrix(matrix.a, matrix.b, matrix.c, matrix.d, matrix.tx, matrix.ty);
		duplicate.name = name;
